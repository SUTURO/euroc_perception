...
int main(int argc, const char *argv[])
{
  ...
  for(iterations=0; iterations<20; iterations++)
  {
    ...
    pcl::transformPointCloud(*model_correspondences, *predicted_model_pts, ct.getRotationMatrixFromPose(x) );
    Eigen::VectorXf y = ct.getVectorFromPointCloud4(predicted_model_pts);
    ...
    {
      changed_pointcloud = ct.transformVariedPose(model_correspondences, x, idx, e);
      Eigen::VectorXf pts_after_variation = ct.getVectorFromPointCloud4(changed_pointcloud);
