...
class Classifier(object):
    ...
    def set_yaml_infos(self, data):
        ...
        rnd_data, rnd_labels = self.create_random_obstacles(number=1000, attributes=self.selected_attributes)#@TODO relative to task
        for each in self.original_objects:
            self.classifier_list[each.name] = self.create_classifier_for_object(each, data+rnd_data, labels+rnd_labels)
    ...
    def create_classifier_for_object(self, object, data, labels):
        new_labels = []
        for each in labels:
            if each != object.name:
    ...
    def randomize_objects(self, objects, number, size_treshold, color_treshold):
        ...
        for object in objects:
            for i in range(0, number):
                r_obj = copy.copy(object)
                if object.has_key('cuboid'):
                    x = object['cuboid'][0]
                    y = object['cuboid'][1]
                    z = object['cuboid'][2]
                    # decimize
                ...
                if object.has_key('volume'):
                    old_vol = object['volume']
                ...
                if object.has_key('color_hsv') or object.has_key('color_rgb'):
    ...
    def create_random_obstacle(self, max_height=2, attributes=[]):
        ...
        if 'color_class' in attributes:
            rnd_obj.append(self.color_class_number_mapping[self.color_classes[random.randrange(len(self.color_classes))]])
